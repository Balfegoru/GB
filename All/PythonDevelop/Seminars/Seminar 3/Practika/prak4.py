...
sum_quantity = 0
sum_price = 0
sum_quantity_result = 0



for key, value in goods.items(): #goods
    # print(f"{key}, {value}")
    for keys,values in store.items(): #store
        if value == keys:
            for v in values: #lists in store
                sum_quantity += v['quantity']
                sum_price += v['price'] * v['quantity']
            print(f"{key} - {sum_quantity} штук, стоимость {sum_price} рублей")
            sum_quantity = 0
            sum_price = 0
